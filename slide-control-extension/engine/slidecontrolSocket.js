...
export default class SlidecontrolSocket {
    ...
    chrome.storage.sync.get(
      ...
      settings => {
        ...
        this.Connection.onerror = error => {
          Logger.error(error);
          alert(
            `Error connecting to slidecontrol server ${settings.backendIP}. Maybe change servers at sc.niveri.xyz/settings`
          );
        };
      }
    );
  ...
  handleMessage(message) {
    ...
    const commandMap = {
      ...
      "controller-disconnected": this.slidecontrolEngine
